...
/**
 * V4 — Vertical scroll with three overlapping staggered columns.
 * Each column starts at a different vertical offset and cards slightly
 * overlap their neighbors horizontally, creating depth. Parallax-like
 * stagger on scroll.
 */
...
const col1: Photo[] = [
  ...
  { label: "Workshop Detail", category: "Detail/macro", aspect: "aspect-[3/4]" },
  { label: "Hill Country", category: "Motorcycle journeys", aspect: "aspect-[16/10]" },
];

const col2: Photo[] = [
  { label: "Devils Den", category: "NWA", aspect: "aspect-[3/5]" },
  { label: "Gravel Road", category: "Motorcycle journeys", aspect: "aspect-[4/3]" },
  { label: "Macro Flower", category: "Detail/macro", aspect: "aspect-[3/4]" },
];

const col3: Photo[] = [
  { label: "Hawksbill Crag", category: "Ozark trails", aspect: "aspect-[4/5]" },
  { label: "Golden Hour", category: "NWA", aspect: "aspect-[16/10]" },
];
...
function PhotoCard({ photo, gradient, delay }: { photo: Photo; gradient: string; delay: number }) {
  return (
    <motion.div
...
export function GalleryV4() {
  return (
    <section id="gallery-v4" className="bg-forest/10 py-24 md:py-32 overflow-hidden">
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          V4 — Vertical scroll, overlapping three-column
        </motion.p>

        {/* Three overlapping columns */}
        <div className="relative flex gap-4 md:gap-6 -mx-4">
          ...
          {/* Column 2 — offset down, overlaps slightly via negative margin */}
          <div className="flex-1 flex flex-col gap-4 md:gap-6 mt-16 md:mt-24 -ml-3 md:-ml-4 z-10">
            {col2.map((photo, i) => (
              <PhotoCard
                key={photo.label}
                photo={photo}
          ...
          {/* Column 3 — offset further, slight left overlap */}
          <div className="flex-1 flex flex-col gap-4 md:gap-6 mt-8 md:mt-12 -ml-3 md:-ml-4">
            {col3.map((photo, i) => (
              <PhotoCard
                key={photo.label}
                photo={photo}
                gradient={gradients[(i + 5) % gradients.length]}
                delay={i * 0.1 + 0.1}
              />
            ))}
          </div>
