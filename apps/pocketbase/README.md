...
| `media_files` / `media_tags`        | Metadata index for `home-server/cdn-gateway`'s NAS library — one row per file (keyed by `path` relative to the library root, served at `cdn.ryanzrau.dev/files/<path>`), with EXIF date/location/camera and tags. Admin-only rules; rows are written only by the cdn-gateway's service account via `pb_hooks/media.pb.js`, and admins can edit tags/description but not on-disk fields.                                                                                                                                                             |
| `media_public`                      | Which cdn-gateway files or folders anyone may view, grouped into named collections (`homepage`). Admin-only rules, edited from ryanzrau.dev/admin. The gateway pulls the rules via a service-only route; `GET /api/custom/media/public/<collection>` lists a collection's files for a page to render, with no auth and display fields only.                                                                                                                                                                                                         |
